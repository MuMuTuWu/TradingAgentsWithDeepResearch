...
async def tavily_search(
    ...
    model_config_with_reasoning = get_model_config_with_reasoning(base_model_config)
    
    summarization_model = init_chat_model(**model_config_with_reasoning).with_structured_output(Summary).with_retry(
...
def get_model_config_with_reasoning(base_config: dict) -> dict:
    """
    为 GPT-5 系列模型自动添加 reasoning_effort 配置
    
    Args:
        base_config: 基础配置字典，应包含 "model" 字段。
                    如果包含 "reasoning_effort" 字段，会使用该值；否则使用默认值。
        
    Returns:
        包含 reasoning_effort 配置的完整配置字典（如果适用）
        
    Note:
        此功能支持 GPT-5 和 o-series 模型（o1, o3, o4等）的 reasoning_effort 参数。
        reasoning_effort 可以设置为: "minimal", "medium", "high"
    """
    ...
    # 如果没有 model 字段，直接返回原配置
    if not model_name:
        return config
    
    # 检查是否为 GPT-5 或 o-series 模型
    model_name_lower = model_name.lower()
    if (model_name_lower.find("gpt-5") != -1 or 
        model_name_lower.find("o1") != -1 or 
        model_name_lower.find("o3") != -1 or
        model_name_lower.find("o4") != -1):
        
        # 如果 base_config 中没有 reasoning_effort 配置，则添加默认配置
        if "reasoning_effort" not in config:
            config["reasoning_effort"] = "minimal"  # 可以设置为 "minimal", "medium", "high"
        
        # 记录日志，表明检测到了 GPT-5 模型并启用了reasoning
        import logging
        effort_level = config.get("reasoning_effort", "minimal")
        logging.info(f"Detected GPT-5/o-series model: {model_name}. "
                    f"Reasoning effort enabled with level: {effort_level}.")
